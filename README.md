...
> **Версия:** 0.7.0 · **Версия ТЗ:** 3.5 · **Заказчик:** ГК «ЕВРОХИМ» · **Дата:** 9 августа 2026 г.
...
- ✅ **Стабильное удержание сессии** (авто-logout при истечении токена, диалог «Сессия истекла»)
...
┌─────────────────────────────────────────────────────────────┐
│                 API (FastAPI) :8000                          │
│  JWT-аутентификация · RBAC · Идемпотентность · Rate limiting │
└──┬───────────┬───────────┬───────────┬───────────┬──────────┘
   │           │           │           │           │
   ▼           ▼           ▼           ▼           ▼
┌────────┐ ┌────────┐ ┌─────────┐ ┌─────────┐ ┌─────────────┐
...
- Все пароли и ключи вынесены из `docker-compose.yml` в файл `.env.docker` (не коммитится в Git)
- Rate limiting на критичных эндпоинтах аутентификации через `slowapi`
...
├── .env.docker               # Секреты инфраструктуры (не коммитится!)
...
Создайте файл `.env.docker` в корне проекта (на основе примера):

```bash
cp .env.example .env.docker
# Отредактируйте пароли на безопасные значения
```

> ⚠️ Файл `.env.docker` содержит пароли PostgreSQL и MinIO. **Не коммитьте его в Git.**

### 3. Запуск Backend + Frontend + инфраструктура
...
### Инфраструктура (`.env.docker` в корне)

| Переменная | Описание | Пример |
| --- | --- | --- |
| `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB` | Учётные данные PostgreSQL | `exsib` |
...
| Пересборка только backend | `docker-compose build --no-cache backend` |
...
| — | **Безопасность: секреты в .env + rate limiting (v0.7.0)** | ✅ Завершён |
...
> ✅ **Решено в v0.7.0 и ранее:** проблема с удержанием сессии в мобильном приложении устранена. Реализованы `AuthEventBus`, автоматический logout при истечении refresh-токена и диалог «Сессия истекла» с возможностью повторного входа.
...
- **Rate limiting** (slowapi): `/auth/login` — 5 запросов/мин, `/auth/refresh` — 10 запросов/мин
- **Секреты вне кода:** пароли PostgreSQL и MinIO вынесены в `.env.docker` (вне Git)
...
| --- | --- | --- |
| **v0.7.0** | 2026-08-09 | Безопасность: секреты в `.env.docker`, rate limiting на auth-эндпоинтах |
| v0.6.1 | — | Стабилизация сессий мобильного приложения |
| v0.6.0 | — | Фотофиксация в мобильном приложении (Этап 8.2) |
| v0.2.0 | — | Docker-контейнеризация, прогресс Этапа 2 |
